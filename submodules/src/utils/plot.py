...
from submodules.src.utils.sim_utils import CURRENT_MAX
...
THICKNESS_CMAP =  LinearSegmentedColormap.from_list("CustomGreens", ['white', '#90be6d'])

CURRENTS_CMAP = LinearSegmentedColormap.from_list("CustomCoolwarm", ['#f9844a', 'white', '#4d908e'])
...
class Plot:
    ...
    def __init__(
        ...
        min_thickness = 0
        max_thickness = round(max(p["thickness"] for p in obstacles), 2)
        sm_thickness = plt.cm.ScalarMappable(cmap=THICKNESS_CMAP, norm=plt.Normalize(vmin=min_thickness, vmax=max_thickness))
        ...
        if self.map:
            ...
            for obs in obstacles:
                self.map_ax.add_patch(
                    patches.Polygon(obs['vertices'], closed=True, fill=False, ec="k", linewidth=0.5)
                )
            ...
            if ship_vertices is not None:
                ...
                self.map_ax.add_patch(
                    patches.Polygon(ship_vertices @ R(ship_pos[2]).T + ship_pos[:2], closed=True, fill=False, color='red')
                )
        ...
        if self.sim:
            ...
            plt.colorbar(sm_thickness, ax=self.obs_ax, orientation="vertical", pad=0.02, label='Ice Thickness (m)')
            ...
            plt.colorbar(sm_thickness, ax=self.sim_ax, orientation="vertical", pad=0.02, label='Ice Thickness (m)')
            ...
            for obs in obstacles:
                self.sim_obs_patches.append(
                    self.sim_ax.add_patch(
                        patches.Polygon(obs['vertices'], closed=True, fill=True, fc=sm_thickness.to_rgba(obs["thickness"]), ec='k', linewidth=0.5)
                    )
                )
            # add the patches for the ice
            self.obs_patches = []
            ...
            if ship_vertices is not None:
                self.ship_patch = self.sim_ax.add_patch(
                    patches.Polygon(ship_vertices @ R(ship_pos[2]).T + ship_pos[:2], closed=True, fill=True,
            ...
            if currents:
                sm_currents = plt.cm.ScalarMappable(cmap=CURRENTS_CMAP, norm=plt.Normalize(vmin=-CURRENT_MAX, vmax=CURRENT_MAX))
                ...
                # Create filled polygon for the current region
                ...
                self.sim_ax.add_patch(current_patch)
        
                plt.colorbar(sm_currents, ax=self.sim_ax, orientation="vertical", pad=0.02, label="Current Value (m/s)")
    ...
    def update_map(self, cost_map: np.ndarray, obstacles: List, ship_vertices=None, ship_pos=None) -> None:
        ...
        for obs in obstacles:
            self.map_ax.add_patch(
                patches.Polygon(obs['vertices'], closed=True, fill=False)
            )
        ...
        if ship_vertices is not None:
            ...
            self.map_ax.add_patch(
                patches.Polygon(ship_vertices @ R.T + ship_pos[:2], closed=True, fill=False, color='red')
            )
    ...
    def animate_sim(self, save_fig_dir=None, suffix=0, im_format='png'):
        self.sim_fig.canvas.blit(self.sim_fig.bbox)
        self.sim_fig.canvas.flush_events()
        self.save(save_fig_dir, suffix, fig='sim', im_format=im_format)
    ...
    def add_ship_patch(ax, vertices, x, y, theta, ec='black', fc='white'):
        ...
        ax.add_patch(
            patches.Polygon(vertices @ R.T + [x, y], closed=True, fill=True, edgecolor=ec, facecolor=fc)
        )
