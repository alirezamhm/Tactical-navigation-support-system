...
class AStarIceNet:
    def __init__(self, weight: float, cmap: CostMap, ke_map: CostMap, prim: Primitives, ship: Ship,
                 swath_dict: Swath, swath_dict_no_padding: Swath, ship_no_padding: Ship,
                 ice_model: IceModel = None, cost_scale: float = 1, cost_log: bool = False, max_thickness: float = 1, max_velocity: float = 43,
                 window_size: tuple[int, int] = (80, 80), device=None, **kwargs):
        ...
        print(self.prim.max_prim, self.ship_no_pad.max_ship_length)
    ...
    def search(self, start: tuple, goal_y: float, 
               occ_map=None, centroids=None, footprint=None, ship_vertices=None, debug=False, 
        ...
        while len(open_set) != 0:
            ...
            ((goal_pos is not None) and (dis_to_goal <= goal_dis or (node[1] + lower_lim) >= self.cmap.occupancy.occ_map_height)):
                ...
                print(len(node_path), "nodes in path")
                ...
                temp = np.zeros(self.cmap.cost_map.shape[0:2], dtype=bool)
                
                if lower_lim < 0:
                    temp[:upper_lim] = full_swath[-lower_lim:]
                else:
            ...
            model_input = None      # batch input with size # (n_neighbors x 6 x W x H)
            for e in edge_set:
                ...
                if 0 < neighbour[0] < self.cost_map.shape[1] and 0 < neighbour[1] < self.cmap.shape[0]:
                    ...
                    if model_input is None:
                        ...
                        model_input = torch.cat((model_input, x), dim=0)        # (n_neighbors, 6, W, H)
            ...
            model_input[:, 1, : , :] = model_input[:, 1, :, :] / self.max_thickness
            model_input[:, 2, : , :] = model_input[:, 2, :, :] / self.max_velocity
            model_input[:, 3, : , :] = model_input[:, 3, :, :] / self.max_velocity
            
            model_input = model_input.to(self.device)
            map_outs, cls_outs, reg_outs = self.ice_model(model_input)                        # (n_neighbors, 4, W, H), (n_neighbors, 1)
            map_outs = map_outs.detach().cpu().numpy()          # (n_neighbors, 4, W, H)        
            cls_outs = torch.sigmoid(cls_outs).detach().cpu().numpy()[:, 0]          # (n_neighbors,)
            reg_outs = reg_outs.detach().cpu().numpy()[:, 0]          # (n_neighbors,)
            
            if self.cost_log:
                costs = np.exp(reg_outs)
            else:
                costs = np.clip(reg_outs, 0, None)
            
            zero_collision_idx = np.where(cls_outs < 0.5)[0]     # indices of neighbors with zero collision cost
            
            costs[zero_collision_idx] = 0
            
            map_outs[:, 1, :, :] = map_outs[:, 1, :, :] * self.max_thickness
            map_outs[:, 2, :, :] = map_outs[:, 2, :, :] * self.max_velocity
            map_outs[:, 3, :, :] = map_outs[:, 3, :, :] * self.max_velocity
            ...
            for neighbor_idx in range(len(neighbors)):
                ...
                swath_cost = costs[neighbor_idx] * self.cost_scale  # use predicted swath cost
                # static_swath_cost = self.get_swath_cost(node, self.swath_arg_dict[key], self.cost_map, self.max_val)

                # add boundary cost
                boundary_map = get_boundary_map(original_shape=original_costmap_shape[0:2], margin=1)
                ...
                if temp_g_score < g_score.get(neighbour, np.inf):
                    ...
                    occ_map_hat = map_outs[neighbor_idx]
                    occ_map_hat = np.transpose(occ_map_hat, (1, 2, 0)) # (4, W, H) -> (W, H, 4)
                    ...
                    swath_cost_memo[neighbour] = swath_cost
    ...
    def build_path(self, path, came_from_by_edge, start, theta_0) -> Tuple[np.ndarray, np.ndarray, dict, list]:
        ...
        """
        full_path = []
        full_swath = np.zeros(self.cost_map.shape[0:2], dtype=bool)
        pt_a = start
        edge_seq = []
    ...
    def get_swath(start_pos, raw_swath, cost_map, max_val) -> np.ndarray:
        ...
        swath = np.zeros(cost_map.shape[0:2], dtype=numba.boolean)
