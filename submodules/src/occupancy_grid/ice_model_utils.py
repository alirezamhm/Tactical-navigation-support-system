...
def crop_window(grid_map, node, win_width, win_height, horizontal_shift, vertical_shift):
    ...
    assert x_low_map >= 0, f"Invalid horizontal shift x_low:  {x}, {x_low_map}, {horizontal_shift}"
    assert x_high_map <= grid_map.shape[1], f"Invalid horizontal shift x_high:  {x}, {x_high_map}, {horizontal_shift}, {grid_map.shape}"
    x_low_win = 0
...
def encode_swath(swath_input, grid_map, node, win_width=36, win_height=36, max_val=29, vertical_shift=0):
    ...
    assert horizontal_shift >= 0 ,f"horizontal shift negative: {horizontal_shift}"

    assert horizontal_shift < win_width, f"Invalid horizontal shift value! {horizontal_shift}, {original_shift}, {win_width}, {node}, ({grid_map.shape[1]}, {grid_map.shape[0]})"
