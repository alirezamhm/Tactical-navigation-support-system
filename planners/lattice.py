...
class LatticeNode(Node):
    ...
    def lattice_planner(self, cfg):
        ...
        while replan_count < cfg.get('max_replan', np.infty) and rclpy.ok():
            ...
            print(f"Path cost: {swath_cost}, length: {length}, cost/length: {swath_cost/length}")
        ...
        print("Finishing")
        if self.debug:
            np.save(f"outputs/lattice/costs_and_lengths_{self.conc}.npy", np.array(costs_and_lengths))
...
if __name__ == '__main__':
    ...
    node = LatticeNode(cfg=cfg, start_trial_idx=args.trial_range[0], id=args.id, conc=args.conc, debug=args.debug)
