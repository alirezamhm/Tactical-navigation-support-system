...
from submodules.src.path_evaluator import IceNetPathEvaluator
...
from submodules.src.a_star_icenet import AStarIceNet
...
class IceNetNode(Node):
    ...
    def icenet_planner(self, cfg):

        if cfg.planner != 'icenet':
        ...
        costmap = CostMap_Occupancy(cfg=cfg, horizon=cfg.a_star.horizon,
                        ship_mass=cfg.ship.mass, channels=4, **cfg.costmap)
        ...
        a_star = AStarIceNet(cmap=costmap,
                ke_map=None,
                prim=prim,
                ship=ship,
        ...
        path_eval = IceNetPathEvaluator(prim=prim, cmap_scale=cfg.costmap.scale, ice_model=ice_model, cost_scale=cfg.cost_scale, cost_log=cfg.cost_log, max_thickness=cfg.max_thickness, max_velocity=cfg.max_velocity,
        ...
        while replan_count < cfg.get('max_replan', np.infty) and rclpy.ok():
            ...
            send_new_path, old_cost, new_cost = path_obj.update_occDiff(old_swath_costs=old_swath_costs, node_path=node_path.T, swath_costs=new_swath_costs, ship_pos=self.ship_pos_scaled,
                                            ...
                                            costmap_scale=cfg.costmap.scale,
                                            drift_threshold=5)
        ...
        print("Finishing")
        if self.debug:
            np.save(f"outputs/v2/icenetV2/costs_and_lengths_{self.conc}.npy", np.array(costs_and_lengths))
...
if __name__ == '__main__':
    ...
    node = IceNetNode(cfg=cfg, start_trial_idx=args.trial_range[0], id=args.id, conc=args.conc, debug=args.debug)

    thread = threading.Thread(target=rclpy.spin, args=(node, ), daemon=True)
    thread.start()

    try:
        node.icenet_planner(cfg=cfg)
